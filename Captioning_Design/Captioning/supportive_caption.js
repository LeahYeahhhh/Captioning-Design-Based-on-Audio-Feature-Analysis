    ...
    let myFont; //自定义字体对象对应样式emphsiseSubtitles
    let currentVideoTime = 0; //存储播放时间
    let font; //全局字体样式
    let wordDrawer;//样式 shout
    //样式2
    class WordDrawer {
        constructor(word, x, y, size) {
            this.word = word;
            this.points = this.getFontOutlinePointsForWord(word, x, y, size);
        }

        getFontOutlinePointsForWord(word, x, y, size) {
            return font.textToPoints(word, x, y, size, { // 使用传递的字体大小
                sampleFactor: 0.3,
                simplifyThreshold: 0,
            });
        }
    ...
    class CustomFont {
        constructor(text, x, y, textSize) {
            ...
            // 创建一个临时的图形对象，用于计算文本尺寸
            let tempGraphic = createGraphics(1, 1);
            tempGraphic.textSize(this.textSize);
            // tempGraphic.textLeading(this.textSize * 1.5);

            // 计算文本的宽度和高度
            this.rectWidth = tempGraphic.textWidth(this.text) + 20;
            this.rectHeight = tempGraphic.textAscent() + tempGraphic.textDescent() + 20;

            // 创建实际图形对象并设置文本属性
            this.graphic = createGraphics(this.rectWidth, this.rectHeight);
            this.graphic.fill(255);
            this.graphic.textSize(this.textSize);
            // this.graphic.textLeading(this.textSize * 1.5);
            this.graphic.textAlign(CENTER, CENTER);
            this.graphic.text(this.text, this.rectWidth / 2, this.rectHeight / 2);
        }

        display() {
            let tileSize = map(0, 0, this.rectWidth, 10, 20);
            for (let i = 0; i < this.rectWidth; i += tileSize) {
                for (let j = 0; j < this.rectHeight; j += tileSize) {
                    let mouseFactor = map(0, 0, this.rectWidth, 0.3, 1);
                    let distortionX = sin(frameCount * 0.05 + (i / tileSize) * 0.15 + j * 0.2) * 5;
                    let distortionY = sin(frameCount * 0.025 + (i / tileSize) * 0.1 + j * 0.1) * 2.5;
                    image(
                        this.graphic,
                        this.x + i - this.rectWidth / 2, // 调整 X 坐标
                        this.y + j - this.rectHeight / 2, // 调整 Y 坐标
                        tileSize,
                        tileSize,
                        i + distortionX,
                        j + distortionY,
    ...
    function shout(fullString, x, y) {
        ...
        //
            if (index !== -1) {
                ...
                wordDrawer = new WordDrawer(foundPart, x2 , y , 50);
                wordDrawer.draw();
                // text(foundPart2, x3, y);
                whisper(foundPart2, x3, y);

                console.log("前-"+foundPart1);
                console.log("查找-"+foundPart);
                console.log("后"+foundPart2);


            } else {
                console.log("找不到"+fullString);
                // text(fullString, x, y);
            ...
            if (currentVideoTime > subtitles[i].startTime && currentVideoTime <= subtitles[i].endTime
                ...
                if (typeof emphsiseSubtitles[emphsiseSubtitleIndex] !== "undefined") {
                    ...
                    if (index !== -1) {
                        ...
                        let y = height / 2 + 300;
                        shout(foundPart1, startX, y);
                        // text(foundPart1, x1, y);
                        myFont = new CustomFont(foundPart, x2 + textWidth(foundPart) / 2, y - 10, 50);
                        myFont.display();
                        // text(foundPart2, x3, y);
                        shout(foundPart2, x3, y);
                        // shout(foundPart2, startX + textWidth(foundPart1) + textWidth(foundPart),  height / 2 + 300);

                    } else {
                        let startX = width / 2 - textWidth(fullString[0]) / 2;
                        // text(fullString[0], startX, height / 2 + 300 - 5);
                    ...
                    if (typeof fullString[1] !== "undefined") {
                        ...
                        if (index !== -1) {
                            ...
                            let startX = width / 2 - textWidth(searchFull) / 2;
                            // text(searchFull, startX, height / 2 + 350);
                    ...
                    if (typeof fullString[1] !== "undefined") {
                        ...
                        // text(fullString[1], startX, height / 2 + 350);
